...
internal partial class ExpediaLodgingApiClient
{
    ...
    {
        return await GetPaginatedApi<GetPropertyContentResponse>(request.ToQueryString("v3/properties/content", QueryStringBuilderFromJsonOptions), cancellationToken: cancellationToken);
    }
    ...
    {
        return await GetPaginatedApi<GetPropertyContentResponse>($"v3/properties/content?token={token}", cancellationToken: cancellationToken);
    }
}