...
public static class ExpediaHelpers
{
    ...
    public static string FindQueryParameterInLink(string link, string parameterName)
    {
        int idx = link.IndexOf('?');
        string query = idx >= 0 ? link[idx..] : "";
        return HttpUtility.ParseQueryString(query).Get(parameterName)!;
    }
