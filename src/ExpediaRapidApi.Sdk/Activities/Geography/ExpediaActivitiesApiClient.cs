...
internal partial class ExpediaActivitiesApiClient
{
    ...
    {
        var url = request.ToQueryString($"v2/regions/{request.RegionId}/activities", QueryStringBuilderFromJsonOptions);
    ...
    {
        var url = request.ToQueryString($"v2/regions/{request.RegionId}/activity-groups", QueryStringBuilderFromJsonOptions);
        return await GetPaginatedApi<List<string>>(url, cancellationToken: cancellationToken);
    }
    ...
    {
        var url = request.ToQueryString($"v2/regions/{request.RegionId}/experiences", QueryStringBuilderFromJsonOptions);
        return await GetPaginatedApi<List<string>>(url, cancellationToken: cancellationToken);
    }
