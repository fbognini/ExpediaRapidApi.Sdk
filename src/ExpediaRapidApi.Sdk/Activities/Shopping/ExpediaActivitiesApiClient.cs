...
internal partial class ExpediaActivitiesApiClient
{
    ...
    {
        var url = WithTrafficProfile(request.ToQueryString("v2/experiences/activities/availability", QueryStringBuilderFromJsonOptions));
    ...
    {
        var url = WithTrafficProfile(request.ToQueryString("v2/experiences/activities/calendars/availability", QueryStringBuilderFromJsonOptions));
    ...
    public async Task<ActivityPriceCheck> PriceCheck(PriceCheckRequest request, PriceCheckOptions options, CancellationToken cancellationToken = default)
    {
        ...
        var url = WithTrafficProfile(request.ToQueryString($"v2/experiences/activities/{request.ActivityId}/price-check", QueryStringBuilderFromJsonOptions));
        return await GetApiAsync<ActivityPriceCheck>(url, requestOptions, cancellationToken);
    }

    public async Task<BookingQuestionDataTypeCatalog> GetBookingQuestionDataTypes(GetBookingQuestionDataTypesRequest request, CancellationToken cancellationToken = default)
    {
        var url = WithTrafficProfile(request.ToQueryString("v2/experiences/booking-questions/data-types", QueryStringBuilderFromJsonOptions));
        return await GetApiAsync<BookingQuestionDataTypeCatalog>(url, cancellationToken: cancellationToken);
    }
}
